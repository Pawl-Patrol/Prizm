...
int getTextLine(char *buf, int maxstrlen, int x, int y, int maxdisplen, unsigned short inmode)
{
	...
	do
	{
		...
		do
		{
				...
				else
				{
					if (krow <= 8 && krow >= 2)
					{
						...
						if (*thisstr != '\0')
						{
							...
							if (valid)
							{
								for (int i = curlen + len; i >= stroffset + len; i--)
								{ //make space to insert this
									buf[i] = buf[i - len];
								}
								for (int i = 0; i < len; i++)
								...
								curlen += len;
								while (stroffset >= dispoffset + maxdisplen - 1)
								{ //adjust the cursor to be on-screen
									redraw = true;
									dispoffset++;
								}
							}
			...
			else if (cursorchangetime + CURSOR_FLASH_RATE < RTC_GetTicks())
			{ //and !key
